...
{
    for (int ch = 0; ch < 10; ++ch) { flashCounters[ch] = 0; samplePositions[ch] = -1.0; channelVelocities[ch] = 1.0f; channelSteps[ch] = 0; sampleNormGains[ch] = 1.0f; }
...
juce::AudioProcessorValueTreeState::ParameterLayout ExtasisRhythmProcessor::createParameterLayout() {
    ...
    params.push_back (std::make_unique<juce::AudioParameterFloat> (juce::ParameterID ("masterVolume", 1), "Master", 0.0f, 4.0f, 1.0f));
...
void ExtasisRhythmProcessor::loadSampleForChannel (int ch, int kit) {
    ...
    sampleNormGains[ch] = 1.0f;
    if (sample.existsAsFile()) {
        auto* reader = formatManager.createReaderFor (sample);
        if (reader) {
            int numSamps = (int) juce::jmin ((juce::int64)reader->lengthInSamples, (juce::int64)(reader->sampleRate * 5.0));
            if (numSamps > 0) {
                juce::AudioBuffer<float> tempBuf ((int)reader->numChannels, numSamps);
...
void ExtasisRhythmProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages) {
    ...
    for (int s = 0; s < buffer.getNumSamples(); ++s) {
        ...
        for (int i = 0; i < 10; ++i) {
            if (localPos[i] >= 0.0 && readerSources[i] != nullptr) {
                ...
                sL *= (env * sampleNormGains[i]); sR *= (env * sampleNormGains[i]);
    ...
    // Transient Shaper
    float tAtt = apvts.getRawParameterValue ("transientAttack")->load();
    float tSus = apvts.getRawParameterValue ("transientSustain")->load();
    if (tAtt != 0.0f || tSus != 0.0f) {
        for (int s = 0; s < buffer.getNumSamples(); ++s) {
            float inL = buffer.getSample (0, s);
            float inR = buffer.getNumChannels() > 1 ? buffer.getSample (1, s) : inL;
            envFastL = envFastL * 0.4f + std::abs (inL) * 0.6f;
            envSlowL = envSlowL * 0.96f + std::abs (inL) * 0.04f;
            float diffL = envFastL - envSlowL;
            float modL = inL + diffL * tAtt * 1.5f + envSlowL * tSus * 0.4f;

            envFastR = envFastR * 0.4f + std::abs (inR) * 0.6f;
            envSlowR = envSlowR * 0.96f + std::abs (inR) * 0.04f;
            float diffR = envFastR - envSlowR;
            float modR = inR + diffR * tAtt * 1.5f + envSlowR * tSus * 0.4f;

            buffer.setSample (0, s, modL);
            if (buffer.getNumChannels() > 1) buffer.setSample (1, s, modR);
        }
    }
    ...
    *masterHpf.state = *juce::dsp::IIR::Coefficients<float>::makeHighPass (currentSampleRate, apvts.getRawParameterValue ("masterHpf")->load(), 0.7f);
    *masterLpf.state = *juce::dsp::IIR::Coefficients<float>::makeLowPass (currentSampleRate, apvts.getRawParameterValue ("masterLpf")->load(), 0.7f);
